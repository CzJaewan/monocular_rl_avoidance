...
class GazeboWorld():
	def __init__(self):
		...
		self.bumper_sub = rospy.Subscriber('turtlebot3_waffle/events/bumper', BumperEvent, self.BumperCallBack)
	...
	def GetDepthImageObservation(self):
		...
		try:
			cv_img = self.bridge.imgmsg_to_cv2(self.depth_image)
		except Exception as e:
			raise e
		try:
			cv_rgb_img = self.bridge.imgmsg_to_cv2(self.rgb_image)
		except Exception as e:
			raise e
	...
	def GetRGBImageObservation(self):
		# ros image to cv2 image
		try:
			cv_img = self.bridge.imgmsg_to_cv2(self.rgb_image)
		except Exception as e:
			raise e
		# resize
		...
		# cv2 image to ros image and publish
		try:
			resized_img = self.bridge.cv2_to_imgmsg(cv_resized_img)
		except Exception as e:
			raise e
	...
	def SetObjectPose(self, name='turtlebot3_waffle', random_flag=False):
		quaternion = tf.transformations.quaternion_from_euler(0., 0., np.random.uniform(-np.pi, np.pi))
		if name is 'turtlebot3_waffle' :
	...
	def GetRewardAndTerminate(self, t):
		...
		if self.GetBump() or np.amin(laser) < 0.30:
			reward = -10.
			terminate = True
			reset = True
		if t > 500:
