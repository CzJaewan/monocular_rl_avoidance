...
class QNetwork(object):
	...
	def __init__(self, sess):
		...
		#print("self.state shape : {}".format(self.state.shape))
		
		# Conv1 layer
		h_conv1 = tf.nn.relu(conv2d(self.state, W_conv1, 8, 8) + b_conv1)
		#print("conv1 shape : {}".format(h_conv1.shape))

		# Conv2 layer
		h_conv2 = tf.nn.relu(conv2d(h_conv1, W_conv2, 2, 2) + b_conv2)
		#print("conv2 shape : {}".format(h_conv2.shape))

		# Conv2 layer
		h_conv3 = tf.nn.relu(conv2d(h_conv2, W_conv3, 1, 1) + b_conv3)
		#print("conv3 shape : {}".format(h_conv3.shape))

		h_conv3_flat = tf.reshape(h_conv3, [-1, H_SIZE])
		#print("h_conv3_flat shape : {}".format(h_conv3_flat.shape))
		...
		#print("h_fc_adv shape : {}".format(h_fc_adv.shape))
		...
		self.out_data = [self.readout, self.V, self.A]
...
def trainNetwork():
	...
	while episode < MAX_EPISODE and not rospy.is_shutdown():
		...
		while not reset and not rospy.is_shutdown():
			...
			#print("state shape : {}".format(depth_imgs_t.shape))
			# choose an action epsilon greedily
			a = sess.run(online_net.out_data, feed_dict = {online_net.state : [depth_imgs_t1]})
			...
			if episode > OBSERVE :
				...
				#print("a_batch :", torch.tensor(a_batch).shape)

				r_batch = [d[2] for d in minibatch]
				r_tensor = torch.tensor(r_batch)
				#print("r_batch :", r_tensor.shape)
