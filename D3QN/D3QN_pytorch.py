...
def trainRun(env, agent, policy_path, args):
	...
	for i_episode in range(args.num_steps):
		...
		#depth_img_t1 = np.reshape(next_depth_img_t1, (DEPTH_IMAGE_HEIGHT, DEPTH_IMAGE_WIDTH, 1))
		
		while not reset and not rospy.is_shutdown():

			#tran_state = Mem.trans_state(state)
			action = agent.select_action(state)

			'''
			#Control
			a_t = np.zeros([ACTIONS])
			if i_episode <= OBSERVE:
			...
			#tran_next_state = Mem.trans_state(next_state)
